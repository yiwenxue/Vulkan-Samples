...
bool HairSimulation::prepare(vkb::Platform &platform)
{
	...
	mannequin->set_texture(load_texture("textures/mannequin_diffuse.png"));
	...
	// glm::vec3 trans             = glm::vec3(0.0, 2.64, 0.08);
	// glm::vec3 rot               = glm::vec3(-38.270, 0.0, 0.0);
	// glm::vec3 scale             = glm::vec3(0.817, 1.158, 1.01);
	...
	// collisionTest->model_matrix = transMat * rotZMat * rotYMat * rotXMat * scaleMat * glm::translate(glm::vec3(-2.0, 1.0, 0.0));
	...
	Collider headCollider      = Collider(glm::vec3(0.0, 2.8, -0.02), glm::vec3(-38.270, 0.0, 0.0), glm::vec3(0.817, 1.158, 1.1));
	...
	std::vector<Collider> colliders{
	    sphereCollider, headCollider, neckCollider, bustCollider, shoulderRCollider, shoulderLCollider};
...
void HairSimulation::simulate(float delta_time)
{
	...
	time_ubo.total_time += delta_time * hair_data.timefactor;
...
void HairSimulation::on_update_ui_overlay(vkb::Drawer &drawer)
{
	...
	{
		drawer.text("simulate time: %fms", hair_system->get_time_ubo().total_time);

		if (drawer.button("reset"))
		{
			hair_data.stiffness     = ori_hair_data.stiffness;
			hair_data.gravityfactor = ori_hair_data.gravityfactor;
			hair_data.damping       = ori_hair_data.damping;
			hair_data.timefactor    = ori_hair_data.timefactor;
		...
		if (drawer.slider_float("Time Speed", &hair_data.timefactor, 0.0, 10.0))
		{
			update_simulation_parameters();
		}
