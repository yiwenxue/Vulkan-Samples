...
void GeomPipeline::destroy()
{
	RenderPipeline::destroy();
}

void GeomPipeline::prepare_pipeline()
{
	...
	shader_stages[0] = get_renderer()->load_shader("hair_simulation/model.vert", VK_SHADER_STAGE_VERTEX_BIT);
	shader_stages[1] = get_renderer()->load_shader("hair_simulation/model.frag", VK_SHADER_STAGE_FRAGMENT_BIT);
...
void GeomPipeline::prepare_texture()
{
}

void GeomPipeline::prepare_uniform_buffer()
{
}

std::vector<VkDescriptorPoolSize> GeomPipeline::regist_pool_size()
{
...
void GeomPipeline::prepare_descriptor_set_layout()
{
...
void GeomPipeline::prepare_descriptor_set()
{
}

void GeomPipeline::draw(VkCommandBuffer cmd, Model *model)
{
	std::vector<VkDescriptorSet> sets{get_renderer()->get_descriptor_set(), model->descriptor_set};
	...
	auto *vertices_buffer = model->vertices_buffer.get();
	auto *indices_buffer  = model->indices_buffer.get();
